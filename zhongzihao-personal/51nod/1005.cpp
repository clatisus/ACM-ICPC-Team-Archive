...
struct Big{ // 仅支持非负整数
    ...
    Big operator * (const Big &p)const{ // 注意长度太长可能会爆 i128，视情况需要将 18 位调小 
...
int main(){
    scanf("%s%s", s1, s2);
    bool flag1 = s1[0] == '-', flag2 = s2[0] == '-';
    char *p1 = s1 + flag1, *p2 = s2 + flag2;
    int len1 = strlen(p1), len2 = strlen(p2);
    Big a(p1, len1), b(p2, len2);
    if (flag1 == flag2){
        if (flag1){
            putchar('-');
        }
