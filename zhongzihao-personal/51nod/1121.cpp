...
ll solve(ll n, ll m, ll p, ll e){
    pii p1 = solve(n, p, e);
    pii p2 = solve(m, p, e);
    pii p3 = solve(n - m, p, e);
    ll cnt = p1.second - p2.second - p3.second;
    if (cnt >= e) return 0;
    return (i128) p1.first * eucinv(p2.first, moder) % moder * eucinv(p3.first, moder) % moder * power[cnt] % moder;
}

ll solve(ll n, ll m){
    ll five = solve(n, m, 5, 18);
    ll mod5 = moder;
    ll two = solve(n, m, 2, 18);
    ll mod2 = moder;
    moder = mod2 * mod5;
    return ((i128) two * mod5 % moder * eucinv(mod5, mod2) + (i128) five * mod2 % moder * eucinv(mod2, mod5)) % moder;
}

int main(){
    ll m, n;
    scanf("%lld%lld", &m, &n);
    ll ans = solve(n + m, n) - solve(n + m, n + 1);
    ans += ans < 0 ? moder : 0;
    printf("%lld\n", ans);
    return 0;
}
